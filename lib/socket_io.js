...
function setupRedux (io) {
  io.on('connect', (s) => {
    ...
    UserHandler.findFromId(currentUserId).then((user) => {
      ...
      removers = removers.concat(LaundryHandler.setupSocket(s, filterGenerator(currentUserId, ({model: {_id}}) => _id)))

      removers = removers.concat(LaundryInvitationHandler.setupSocket(s, filterGenerator(currentUserId, ({model: {laundry}}) => laundry)))

      removers = removers.concat(MachineHandler.setupSocket(s, filterGenerator(currentUserId, ({model: {laundry}}) => laundry)))

      removers = removers.concat(BookingHandler.setupSocket(s, filterGenerator(currentUserId, ({model: {laundry}}) => laundry)))
