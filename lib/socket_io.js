...
function socketOn (socket, event, action) {
  ...
  socket.on(event, function (opts) {
    ...
    action.apply(this, arguments.slice(1))
      .catch(error.logError)
      .then(() => trans.end())
  })
}

function socketOnLaundry (socket, currentUser, event, action) {
  return socketOn(socket, event, fetchLaundryGenerator(currentUser, action))
}
 ...
 * @returns {Promise}
 */
function emitResult (socket, promise, action, jobId) {
  if (!jobId) return promise.then(result => socket.emit('action', action(result)))
  return emitResults(socket, [promise, jobId], [action, actions.finishJob])
}
 ...
 * @param {int=} jobId
 */
function emitResults (socket, promises, acts, jobId) {
  const promise = typeof promises.then === 'function' ? promises : Promise.all(promises)
  if (jobId) return emitResults(socket, promise.then(results => results.concat(jobId)), acts.concat(actions.finishJob))
  return promise
    .then(results => results
      .map((r, i) => acts[i](r)))
    .then(actions => socket.emit('actions', actions))
}

function fetchLaundryGenerator (currentUserId, fn) {
  return function (laundryId) {
    return fetchLaundry(currentUserId, laundryId)
      .then(laundry => {
        if (!laundry) return
        fn.apply([laundry].concat(arguments.slice(1)))
      })
  }
}
...
function setupFunctions (socket, currentUserId) {
  socketOnLaundry(socket, currentUserId, 'listBookingsInTime',
    (laundry, from, to) =>
      emitResult(socket, laundry.fetchBookings(new Date(from), new Date(to)), actions.listBookings))

  socketOnLaundry(socket, currentUserId, 'listBookingsForUser',
    (laundry, userId, filter) =>
      emitResult(
        ...
        actions.listBookingsForUser))

  socketOnLaundry(socket, currentUserId, 'listUsersAndInvites',
    laundry => {
      debug('Emitting list users and invites')
      return emitResults(socket, [
        laundry.fetchUsers(),
        laundry.fetchInvites()
      ], [
        actions.listUsers,
        actions.listInvites
      ])
  ...
  socketOnLaundry(socket, currentUserId, 'listMachinesAndUsers',
    laundry => {
      debug('Emitting list users and machines')
      return emitResults(socket, [
        laundry.fetchUsers(),
        laundry.fetchMachines()
      ], [
        actions.listUsers,
  ...
  socketOnLaundry(socket, currentUserId, 'listMachines',
    laundry => {
