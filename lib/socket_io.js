...
function setupAdminFunctions (socket) {
  ...
  socketOn(socket, 'updateStats', () => {
    ...
    const promisedStats = Promise
      .all([
        LaundryHandler.fetchEvents(LaundryHandler.eventFilters.within24HoursDeleteUpdate),
      ...
      .then(([
        ...
        laundryEvents: laundryEvents.map(e => e.reduxModel),
        laundryCount,
        userCount,
        bookingCount,
        machineCount
...
function setupRedux (io) {
  ...
  io.on('connect', socket => {
    ...
    UserHandler.findFromId(currentUserId).then((user) => {
      ...
      if (user.model.role === 'admin') setupAdminFunctions(socket, currentUserId)
