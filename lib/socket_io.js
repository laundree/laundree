    ...
    UserHandler.findFromId(currentUserId).then((user) => {
      ...
      s.on('listBookingsForUser', (laundryId, userId, filter) => {
        LaundryHandler.findFromId(laundryId).then((laundry) => {
          ...
          BookingHandler
            .find(Object.assign({}, filter, {machine: {$in: laundry.model.machines}, owner: userId}), {sort: {from: 1, to: 1}})
            .then((bookings) => s.emit('action', actions.listBookingsForUser(bookings)))
        })
      })
      s.on('disconnect', () => {
        debug('Disconnecting')
