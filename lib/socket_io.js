...
function setupFunctions (socket) {
  defineLaundrySocketFunction(
    socket,
    'listBookingsInTime',
    (laundry, from, to) => laundry.fetchBookings(new Date(from), new Date(to)).then(actions.listBookings))
...
function setupAdminFunctions (socket) {
  ...
  defineSocketFunction(
    socket,
    'listLaundries',
    () => LaundryHandler.find().then(actions.listLaundries))

  defineSocketFunction(
    socket,
    'listUsers',
    () => UserHandler.find().then(actions.listUsers)
  )
  defineSocketFunction(
    socket,
    'updateStats',
